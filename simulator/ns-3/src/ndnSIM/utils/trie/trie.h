        ...
        if (item == trieNode->children_.end ())
          {
            ...
            //typename unordered_set::iterator it=trieNode->children_.insert (*newNode);
            //std::pair< typename unordered_set::iterator, bool > ret =std::make_pair(it,true);
            //endmaz
            trieNode = &(*ret.first);
          }
        else
...
/*
         ...
         std::pair<typename unordered_set::iterator,typename unordered_set::iterator> mymultiset = trieNode->children_.equal_range(subkey);
    	//auto pair1 = trieNode->children_.equal_range(subkey);
         //typename trie::unordered_set::iterator It;
    	typename unordered_set::iterator item = trieNode->children_.end ();

         while (true) {
        	 if(mymultiset.first==trieNode->children_.end()&&mymultiset.second==trieNode->children_.end()) break;
        	 if(mymultiset.first->payload_== PayloadTraits::empty_payload){
        		 if(mymultiset.first == mymultiset.second) break;
        		 mymultiset.first++;

        		 continue;
        	 }
        	 bool cond=pred (mymultiset.first->payload_);
        	 if(cond) {
        		 std::cout<<"1";
        		 break;
        	 }
    	   ...
    	   // std::string str="1";//(mymultiset.first)->payload_->GetInterest()->GetName().toUri();
        	 if(mymultiset.first == mymultiset.second) break;
    	    mymultiset.first++;
    	 }

         item=mymultiset.first;
*/
...
private:
  ...
  //maz
 // typedef boost::intrusive::unordered_multiset< trie, member_hook > unordered_Multiset;
  //maz
  typedef typename unordered_set::bucket_type   bucket_type;
  typedef typename unordered_set::bucket_traits bucket_traits;

  //typedef typename unordered_Multiset::bucket_type   bucket_type2;
 // typedef typename unordered_Multiset::bucket_traits bucket_traits2;
