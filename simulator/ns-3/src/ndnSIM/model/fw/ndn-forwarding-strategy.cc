                                ...
                                Ptr<Interest> interest)
{
	...
	int seq=(int)interest->GetName ().get (-1).toSeqNum ();

*/


  NS_LOG_FUNCTION (inFace << interest->GetName ());
  //std::cout<<interest->GetName().toUri()<<"\n";
  m_inInterests (interest, inFace);

  Ptr<pit::Entry> pitEntry = m_pit->Lookup (*interest);
  int a=interest->GetNonce();
  int b=0;
  if(pitEntry != 0)
	  b=pitEntry->GetNounce();
  //Ptr<pit::Entry> pitEntry;// = m_pit->Lookup (*interest);
...
//}
 ...
 // ndn::AppDelayTracer::GetIncreaseObjRequest(seq,nodeId);
  Ptr<Data> contentObject;
  contentObject = m_contentStore->Lookup (interest);
  if (contentObject != 0)
    {
	  //std::cout<<interest->GetName().toUri()<<" satisfied by node "<<inFace->GetNode()->GetId()<<"\n";
  ...
  if (similarInterest && ShouldSuppressIncomingInterest (inFace, interest, pitEntry))
    {
	  ...
	  ndn::AppDelayTracer::GetIncreaseAggCount(true);
	  //std::cout<<nodeId<<"\n";


	 // ndn::AppDelayTracer::GetIncreaseAggCountPerObj(seq,nodeId);
/*	  if(nodeId<21)
		  ndn::AppDelayTracer::GetIncreaseAggCount(false);*/
  ...
  //std::cout<<Simulator::Now()<<" : "<<interest->GetName().toUri()<<" propagated by node "<<inFace->GetNode()->GetId()<<"\n";
...
ForwardingStrategy::OnData (Ptr<Face> inFace,
                            Ptr<Data> data)
{
	//std::cout<<data->GetNamePtr()->toUri()<<" On Data "<<inFace->GetNode()->GetId()<<"\n";
  NS_LOG_FUNCTION (inFace << data->GetName ());
  m_inData (data, inFace);


//  uint32_t nodeId=inFace->GetNode()->GetId();
  //NameValue dataName= data->GetName();
  //	uint32_t interestNounce=interest->GetNonce();

  	//int a=0;
  	//if(nodeId==0&&dataName.GetReferenceCount())
  	//	a++;
  // Lookup PIT entry
  ...
  if (pitEntry == 0)
    ...
    {
	  //ndn::AppDelayTracer::GetIncreaseHopCount(1,true);
	  /*if(pitEntry->GetInterest()->GetConsumerNode()==inFace->GetNode()->GetId()){
		  int hopCnt=0;
		  FwHopCountTag hopCount;
		  Ptr<Packet> pkt = data->GetPayload()->Copy ();
		  bool found= pkt->RemovePacketTag(hopCount);
		  if(found){
			  hopCnt=hopCount.Get();
		  }
		  //ndn::AppDelayTracer::GetIncreaseHopCount(hopCnt,true);
	  }*/

	  //if(pitEntry->GetInterest()->GetNonce()==inFace->GetNode()->GetId()){

		  bool cached = m_contentStore->Add (data);
		  DidReceiveSolicitedData (inFace, data, cached);
		  if(cached){
			//  std::cout<<data->GetNamePtr()->toUri()<<" cached by node "<<inFace->GetNode()->GetId()<<"\n";
			 // modifyRouting(data,inFace);
		  }
	  //}
    }
 // bool flag=false;
...
void
ForwardingStrategy::modifyRouting(Ptr<Data> data, Ptr<Face> inFace){
	//Ptr<AppHelper> producer
				if(data->GetName().toUri()=="/pre0/%00'%10")
					return;
			  Ptr<GlobalRouter> gr = (inFace->GetNode())->GetObject<GlobalRouter> ();
			  Ptr<ndn::Name> name = Create<ndn::Name> ();
			  name->append(data->GetNamePtr()->toUri());
			  //gr->AddLocalPrefix(name);

			  //std::cout<<data->GetNamePtr()->toUri()<<" "<<inFace->GetNode()->GetId()<<"\n";

			  ndn::GlobalRoutingHelper grh;
			  //grh.AddOrigin("/pre0",nodes[0]);
			  grh.AddOrigin(data->GetName().toUri(),inFace->GetNode());
			  grh.CalculateRoutes(false);
                                            ...
                                            Ptr<pit::Entry> pitEntry)
{
  ...
  BOOST_FOREACH (const pit::IncomingFace &incoming, pitEntry->GetIncoming ())
    {
	  if(flag){
		  ndn::AppDelayTracer::GetIncreaseHopCount(hopCnt, false);
	  }
	  else{
		  ndn::AppDelayTracer::GetIncreaseHopCount(1, false);
		  ndn::AppDelayTracer::GetIncreaseHopCount(1, true);
	  }
	  flag=true;
      bool ok = incoming.m_face->SendData (data);

      //std::cout<<data->GetNamePtr()->toUri()<<" satisfied2 by node "<<inFace->GetNode()->GetId()<<"\n";
                                       ...
                                       Ptr<pit::Entry> pitEntry)
{
	...
	ndn::AppDelayTracer::GetIncreasePrefixRequest(inFace->GetNode()->GetId(), interest->GetName().toUri().substr(0,7));
  bool isRetransmitted = m_detectRetransmissions && // a small guard
                         DetectRetransmittedInterest (inFace, interest, pitEntry);
