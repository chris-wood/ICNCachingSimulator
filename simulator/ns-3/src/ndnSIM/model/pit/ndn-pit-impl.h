...
PitImpl<Policy>::Lookup (const Data &header)
{
  /// @todo use predicate to search with exclude filters
	...
	//maz
  //typename super::iterator item = super::longest_prefix_match_if (header.GetName (), NounceMatch(header.GetNounce()));
//mazend
  if (item == super::end ())
    return 0;
  else
...
PitImpl<Policy>::Lookup (const Interest &header)
{
  ...
  //maz
  //uint32_t nnc=header.GetNonce();
  //boost::tie (foundItem, reachLast, lastItem) = super::getTrie ().find_if (header.GetName (),NounceMatch(header.GetNonce()));
  //boost::tie (foundItem, reachLast, lastItem) = super::getTrie ().find_if (header.GetName (),EntryIsNotEmpty ());
//mazEnd
  if (!reachLast || lastItem == super::end ())
    return 0;
  else{
	 /* std::string str1 = header.GetName().toUri();
	  std::string str2=lastItem->payload ()->GetInterest()->GetName().toUri();
	 uint32_t s=lastItem->payload ()->GetNounce();*/
	  //mazz//for nounce matching
