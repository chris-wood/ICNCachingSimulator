...
struct NounceMatch
{
	NounceMatch (uint32_t n)
	     : value(n)
	   {}
  bool
  operator () (Ptr<Entry> entry)
  {
	  /*uint32_t s=entry->GetNounce();
	  bool bo1=entry->GetNounce()==value;
	  bool bo2=!entry->GetIncoming ().empty ();*/
      return (entry->GetNounce()==value&&!entry->GetIncoming ().empty ());
	 // return true;
  }
