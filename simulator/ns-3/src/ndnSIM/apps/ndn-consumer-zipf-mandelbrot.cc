...
ConsumerZipfMandelbrot::GetTypeId (void)
{
 ...
 /*  .AddAttribute ("Randomize", "Type of send time randomization: none (default), uniform, exponential",
				  StringValue ("customExp"),
				  MakeStringAccessor (&ConsumerZipfMandelbrot::SetRandomize, &ConsumerZipfMandelbrot::GetRandomize),
				  MakeStringChecker ())*/
...
ConsumerZipfMandelbrot::SendPacket() {
  ...
  ndn::AppDelayTracer::GetIncreaseObjRequest(seq);
  Ptr<Name> nameWithSequence = Create<Name> (m_interestName);
  nameWithSequence->appendSeqNum (seq);

 // std::cout<<nameWithSequence->toUri()<<"\n";
