...
#include "ndn-consumer-zipf-mandelbrot2.h"
...
NS_LOG_COMPONENT_DEFINE ("ndn.ConsumerZipfMandelbrot2");

namespace ns3 {
namespace ndn {
static int intcnt=0;

NS_OBJECT_ENSURE_REGISTERED (ConsumerZipfMandelbrot2);

TypeId
ConsumerZipfMandelbrot2::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ndn::ConsumerZipfMandelbrot2")
    .SetGroupName ("Ndn")
    .SetParent<ConsumerCbr> ()
    .AddConstructor<ConsumerZipfMandelbrot2> ()

    .AddAttribute ("NumberOfContents", "Number of the Contents in total",
                   StringValue ("100"),
                   MakeUintegerAccessor (&ConsumerZipfMandelbrot2::SetNumberOfContents, &ConsumerZipfMandelbrot2::GetNumberOfContents),
                   MakeUintegerChecker<uint32_t> ())

    .AddAttribute ("q", "parameter of improve rank",
                   StringValue ("0.7"),
                   MakeDoubleAccessor (&ConsumerZipfMandelbrot2::SetQ, &ConsumerZipfMandelbrot2::GetQ),
                   MakeDoubleChecker<double> ())
    
    .AddAttribute ("s", "parameter of power",
                   StringValue ("0.7"),
                   MakeDoubleAccessor (&ConsumerZipfMandelbrot2::SetS, &ConsumerZipfMandelbrot2::GetS),
                   MakeDoubleChecker<double> ())
    ;
...
ConsumerZipfMandelbrot2::ConsumerZipfMandelbrot2()
...
void
ConsumerZipfMandelbrot2::SetNumberOfContents (uint32_t numOfContents)
{
  m_N = numOfContents;
...
uint32_t
ConsumerZipfMandelbrot2::GetNumberOfContents () const
{
  return m_N;
}

void
ConsumerZipfMandelbrot2::SetQ (double q)
{
  m_q = q;
...
double
ConsumerZipfMandelbrot2::GetQ () const
{
  return m_q;
}

void
ConsumerZipfMandelbrot2::SetS (double s)
{
  m_s = s;
  SetNumberOfContents (m_N);
}
...
double
ConsumerZipfMandelbrot2::GetS () const
{
  return m_s;
}

void
ConsumerZipfMandelbrot2::SendPacket() {
  if (!m_active) return;
  ...
  if (seq == std::numeric_limits<uint32_t>::max ()) //no retransmission
    {
      ...
      seq = ConsumerZipfMandelbrot2::GetNextSeq();
      m_seq ++;
    }
  ...
  ConsumerZipfMandelbrot2::ScheduleNextPacket ();
}


uint32_t
ConsumerZipfMandelbrot2::GetNextSeq()
{
  ...
  if (m_firstTime)
    {
      m_sendEvent = Simulator::Schedule (Seconds (0.0),
                                         &ConsumerZipfMandelbrot2::SendPacket, this);
      m_firstTime = false;
    }
