...
NS_LOG_COMPONENT_DEFINE ("ndn.DCRRoutingHelper");
...
void
DCRRoutingHelper::Install (Ptr<Node> node)
{
  NS_LOG_LOGIC ("Node: " << node->GetId ());

  Ptr<L3Protocol> ndn = node->GetObject<L3Protocol> ();
  NS_ASSERT_MSG (ndn != 0, "Cannot install DCRRoutingHelper before Ndn is installed on a node");
...
void
DCRRoutingHelper::Install (Ptr<Channel> channel)
{
...
void
DCRRoutingHelper::Install (const NodeContainer &nodes)
{
...
void
DCRRoutingHelper::AddOrigin (const std::string &prefix, Ptr<Node> node)
{
  Ptr<GlobalRouter> gr = node->GetObject<GlobalRouter> ();
  NS_ASSERT_MSG (gr != 0,
...
void
DCRRoutingHelper::AddOrigins (const std::string &prefix, const NodeContainer &nodes)
{
  for (NodeContainer::Iterator node = nodes.Begin ();
       node != nodes.End ();
       node++)
...
void
DCRRoutingHelper::AddOrigin (const std::string &prefix, const std::string &nodeName)
{
...
void
DCRRoutingHelper::AddOriginsForAll ()
{
...
void
DCRRoutingHelper::CalculateRoutes (bool invalidatedRoutes/* = true*/)
{
  /**
...
void
DCRRoutingHelper::ModifyFaces(Ptr<Face> inFace, Ptr<const Data> data){

}


void
DCRRoutingHelper::CalculateAllPossibleRoutes (bool invalidatedRoutes/* = true*/)
{
  /**
   * Implementation of route calculation is heavily based on Boost Graph Library
   * See http://www.boost.org/doc/libs/1_49_0/libs/graph/doc/table_of_contents.html for more details
