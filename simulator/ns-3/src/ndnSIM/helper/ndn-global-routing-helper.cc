	  ...
	  if (i->first == source)
	      ...
	      else
		{
                  BOOST_FOREACH (const Ptr<const Name> &prefix, i->first->GetLocalPrefixes ())
                    {
                	 // std::cout<<" prefix " << prefix->toUri() << " reachable via face " << *i->second.get<0> ()
...
void
GlobalRoutingHelper::CustomRoutes(){
...
/*		    	  if((*node)->GetId ()==4){
		    	 					 // return;
		    	 					  Ptr<Node> node1=NodeList::GetNode(4);
		    	 					  for(int i=0;i<node1->GetNDevices();i++){
		    	 						  Ptr<Channel> ch=node1->GetDevice(i)->GetChannel();
		    	 						  for(int k=0;k<ch->GetNDevices();k++){
		    	 							  if(ch->GetDevice(k)->GetNode()->GetId()==3){
		    	 									//Ptr<Ipv4> stack = node->GetObject<Ipv4> ();
		    	 									Ptr<ndn::L3Protocol> l3 = node1->GetObject<ndn::L3Protocol> ();
		    	 									const string str="prefix1";
		    	 									Ptr<Name> name = Create<Name>();
		    	 									name->append(str);
		    	 									const Ptr<const Name> &prefix=name;
		    	 									//prefix->append(str);
		    	 								  Ptr<fib::Entry> entry = fib->Add (prefix, l3->GetFace(i), 1 );
		    	 							  }
		    	 							  if(ch->GetDevice(k)->GetNode()->GetId()==3){
		    	 								//Ptr<Ipv4> stack = node->GetObject<Ipv4> ();
		    	 								Ptr<ndn::L3Protocol> l3 = node1->GetObject<ndn::L3Protocol> ();
		    	 								const string str="prefix2";
		    	 								Ptr<Name> name = Create<Name>();
		    	 								name->append(str);
		    	 								const Ptr<const Name> &prefix=name;
		    	 								//prefix->append(str);
				  ...
				  if((*node)->GetId ()==7){
					 // return;
					  Ptr<Node> node1=NodeList::GetNode(7);
					  for(int i=0;i<node1->GetNDevices();i++){
						  Ptr<Channel> ch=node1->GetDevice(i)->GetChannel();
						  for(int k=0;k<ch->GetNDevices();k++){
							  if(ch->GetDevice(k)->GetNode()->GetId()==6){
									//Ptr<Ipv4> stack = node->GetObject<Ipv4> ();
									Ptr<ndn::L3Protocol> l3 = node1->GetObject<ndn::L3Protocol> ();
									const string str="prefix1";
									Ptr<Name> name = Create<Name>();
									...
									const Ptr<const Name> &prefix=name;
									//prefix->append(str);
								  Ptr<fib::Entry> entry = fib->Add (prefix, l3->GetFace(i), 0 );
							  }
							  if(ch->GetDevice(k)->GetNode()->GetId()==8){
								//Ptr<Ipv4> stack = node->GetObject<Ipv4> ();
								Ptr<ndn::L3Protocol> l3 = node1->GetObject<ndn::L3Protocol> ();
								const string str="prefix2";
								Ptr<Name> name = Create<Name>();
	                  ...
	                  BOOST_FOREACH (const Ptr<const Name> &prefix, i->first->GetLocalPrefixes ())
	                    {
	                	  std::cout<<i->first->GetObject<Node> ()->GetId ();
	                	 /* if((*node)->GetId ()==1&&!prefix->toUri().compare("prefix1")){
	                	  	                		  continue;
	                	  	                	  }*/
	                      NS_LOG_DEBUG (" prefix " << prefix << " reachable via face " << *i->second.get<0> ()
	                                    << " with distance " << i->second.get<1> ()
	                      ...
	                      //if(i->second.get<1> () ==1) return;//test only, checking nacks
...
void
GlobalRoutingHelper::FillRoutes(){

}

void
GlobalRoutingHelper::ModifyFaces(Ptr<Face> inFace, Ptr<const Data> data){
