...
RoutingProtocol::RouteInput (Ptr<const Packet> p,
                             ...
                             ErrorCallback ecb)
{
	ns3::dsdv::RoutingTable::faceChange(1,1,1);
...
RoutingProtocol::NotifyInterfaceUp (uint32_t i)
{
  ...
  Ptr<NetDevice> localNetDevice=m_ipv4->GetNetDevice(i);
  Ptr<Channel> channel=localNetDevice->GetChannel();

  /*for(int j=0; j<channel->GetNDevices(); j++){

	  Ptr<NetDevice> remoteDevice = channel->GetDevice (j);
	  if (remoteDevice != localNetDevice)
		  ns3::dsdv::RoutingTable::faceChange(true, localNetDevice->GetNode()->GetId(),channel->GetDevice(j)->GetNode()->GetId());
  }
*/
...
RoutingProtocol::NotifyInterfaceDown (uint32_t i)
{
  ...
  Ptr<NetDevice> localNetDevice=m_ipv4->GetNetDevice(i);
    ...
    /*for(int j=0; j<channel->GetNDevices(); j++){
  	  ...
  	  Ptr<NetDevice> remoteDevice = channel->GetDevice (j);
  	  if (remoteDevice != localNetDevice)
  		  ns3::dsdv::RoutingTable::faceChange(false, localNetDevice->GetNode()->GetId(),channel->GetDevice(j)->GetNode()->GetId());
    }*/
