...
void
RoutingTable::faceChange(bool isUp, int NodeId1, int NodeId2){
	...
	for(int i=0;i<nodeCount;i++){
		...
		for(int j=0;j<node->GetNDevices();j++){
			...
			if(!netDevice->IsLinkUp()) continue;
			Ptr<Channel> channel=netDevice->GetChannel();
			if(channel==0) continue;
			  for(int k=0; k<channel->GetNDevices(); k++){
				  Ptr<NetDevice> remoteDevice = channel->GetDevice (k);
				  if (remoteDevice != netDevice)
					  tbl[netDevice->GetNode()->GetId()][remoteDevice->GetNode()->GetId()]=true;
			  }
		}
	}
...
*/
  //tbl[NodeId1][NodeId2]=isUp;
  std::cout<<"first: "<<NodeId1<<" second: "<<NodeId2<<"   "<< Simulator::Now()<<"\n";
  for(int i=0;i<nodeCount;i++){
	  for(int j=0;j<nodeCount;j++){
		  std::cout<< tbl[i][j]<< " ";
	  }
