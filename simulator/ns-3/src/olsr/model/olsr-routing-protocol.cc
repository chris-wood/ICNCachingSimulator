...
RoutingProtocol::RoutingTableComputation ()
{
	...
	//if(rand() % ((NodeList::GetNNodes())*100)!=0) return;
  NS_LOG_DEBUG (Simulator::Now ().GetSeconds () << " s: Node " << m_mainAddress
                                                << ": RoutingTableComputation begin...");
// if(OlsrHelper::flag)

	flag=true; IntervalRoutingTableComputation();return;
...
void
RoutingProtocol::IntervalRoutingTableComputation (){



	if(Simulator::Now().GetSeconds()>19) return;
	...
	Ptr<Node> node[1000];
		for(int i=0;i<NodeList::GetNNodes();i++){
			node[i]=NodeList::GetNode(i);
			  Ptr<Ipv4> stack = node[i] -> GetObject<Ipv4> ();
			  Ptr<Ipv4RoutingProtocol> rp_Gw = (stack->GetRoutingProtocol ());
			  Ptr<olsr::RoutingProtocol> olsr_Gw = DynamicCast<olsr::RoutingProtocol> (rp_Gw);
			  olsr_Gw->Clear();
		}

		std::ifstream routingFile;
		routingFile.open ("routes.txt");
		std::string line;
		if (routingFile.is_open())
		{
			while ( getline (routingFile,line) )
			{
				...
				int cur_node,dest,nextHop,ne_inface,des_inface;
				if (!(iss >> cur_node >> dest >> nextHop >> ne_inface >> des_inface )) { break; }
				Ptr<Ipv4> cu_stack = node[cur_node] -> GetObject<Ipv4> ();
				Ptr<Ipv4RoutingProtocol> cu_rp = (cu_stack->GetRoutingProtocol ());
				Ptr<olsr::RoutingProtocol> cu_olsr_Gw = DynamicCast<olsr::RoutingProtocol> (cu_rp);

				Ptr<Ipv4> des_stack = node[dest] -> GetObject<Ipv4> ();
				Ptr<Ipv4RoutingProtocol> des_rp = (des_stack->GetRoutingProtocol ());
				Ptr<olsr::RoutingProtocol> des_olsr_Gw = DynamicCast<olsr::RoutingProtocol> (des_rp);

				Ptr<Ipv4> ne_stack = node[nextHop] -> GetObject<Ipv4> ();
				Ptr<Ipv4RoutingProtocol> ne_rp = (ne_stack->GetRoutingProtocol ());
				Ptr<olsr::RoutingProtocol> ne_olsr_Gw = DynamicCast<olsr::RoutingProtocol> (ne_rp);


				Ipv4InterfaceAddress next_add= ne_stack->GetAddress(1,0);
				Ipv4InterfaceAddress des_add= des_stack->GetAddress(1,0);


				const TopologySet &des_topology = des_olsr_Gw->m_state.GetTopologySet ();
				if(des_topology.begin()==des_topology.end()) continue;
				TopologySet::const_iterator des_it = des_topology.begin ();

				const TopologyTuple &des_top_tup = *des_it;

				const TopologySet &ne_topology = ne_olsr_Gw->m_state.GetTopologySet ();
				if(ne_topology.begin()==ne_topology.end()) continue;
				TopologySet::const_iterator ne_it = ne_topology.begin ();
				const TopologyTuple &ne_top_tup = *ne_it;


				//const TopologyTuple &des_top_tup = des_olsr_Gw->m_state.m_topologySet.begin();
				//const TopologyTuple &ne_top_tup = ne_olsr_Gw->m_state.m_topologySet.begin();


				//std::cout<<"add entry :"<<cur_node<<" "<<des_add.GetLocal()<<""<<next_add.GetLocal()<<" "<<ne_inface<<"\n";
				//cu_olsr_Gw->AddEntry(des_top_tup.destAddr,ne_top_tup.destAddr,ne_inface,3);
				cu_olsr_Gw->AddEntry(des_add.GetLocal(),next_add.GetLocal(),ne_inface,3);
				//olsr_Gw->AddEntry(intervalRT[i].destAddr,intervalRT[i].nextAddr,intervalRT[i].interface,intervalRT[i].distance);
		...
		for(int i=0;i<(sizeof(intervalRT)/sizeof(intervalRT[0]));i++){

			Ptr<Ipv4> stack = node[intervalRT[i].nodeId] -> GetObject<Ipv4> ();
			Ptr<Ipv4RoutingProtocol> rp_Gw = (stack->GetRoutingProtocol ());
			Ptr<olsr::RoutingProtocol> olsr_Gw = DynamicCast<olsr::RoutingProtocol> (rp_Gw);
			olsr_Gw->AddEntry(intervalRT[i].destAddr,intervalRT[i].nextAddr,intervalRT[i].interface,intervalRT[i].distance);
		}
                           ...
                           uint32_t distance)
{
  ...
  ofs<<"add entry :"<<m_mainAddress<<" "<<dest<<" "<<next<<" "<<interface<<"\n";
  ofs.close();
...
// if(OlsrHelper::flag)
	...
	if (olsr_interval)
	{
		//std::cout<<"Interval!\n";
		flag=true;
		IntervalRoutingTableComputation();
		return;
	}
...
RoutingProtocol::IntervalRoutingTableComputation (){
	...
	//if(Simulator::Now().GetSeconds()<14) return;
	//if(Simulator::Now().GetSeconds()>19) return;
	//std::cout<<Simulator::Now().GetSeconds()<<"\n";

	if(Simulator::Now().GetSeconds()<5)
	{
		//if(rand() % ((NodeList::GetNNodes())*10)!=0)
		return;
	}
	//if(rand() % (NodeList::GetNNodes())/10 != 0) return;
	...
	int a=sizeof(routes_currNode)/sizeof(routes_currNode[0]);
std::cout<<a<<"\n";
	for(int l=0;l<a;l++)
	{
		Ptr<Ipv4> cu_stack = node[routes_currNode[l]] -> GetObject<Ipv4> ();
		Ptr<Ipv4RoutingProtocol> cu_rp = (cu_stack->GetRoutingProtocol ());
		Ptr<olsr::RoutingProtocol> cu_olsr_Gw = DynamicCast<olsr::RoutingProtocol> (cu_rp);

		Ptr<Ipv4> des_stack = node[routes_destNode[l]] -> GetObject<Ipv4> ();
		Ptr<Ipv4RoutingProtocol> des_rp = (des_stack->GetRoutingProtocol ());
		Ptr<olsr::RoutingProtocol> des_olsr_Gw = DynamicCast<olsr::RoutingProtocol> (des_rp);

		Ptr<Ipv4> ne_stack = node[routes_nextHop[l]] -> GetObject<Ipv4> ();
		Ptr<Ipv4RoutingProtocol> ne_rp = (ne_stack->GetRoutingProtocol ());
		Ptr<olsr::RoutingProtocol> ne_olsr_Gw = DynamicCast<olsr::RoutingProtocol> (ne_rp);
		...
		cu_olsr_Gw->AddEntry(des_add.GetLocal(),next_add.GetLocal(),routes_ne_inface[l],3);
	}
}
